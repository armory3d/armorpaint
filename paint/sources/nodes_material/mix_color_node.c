...
char *mix_color_node_vector(ui_node_t *node, ui_node_socket_t *socket) {
	...
	if (string_equals(blend, "MIX")) {
		...
		parser_material_write(parser_material_kong, string_tmp("if (%s.r < 0.5) { %s = 2.0 * %s.r * %s.r; } else { %s = 1.0 - 2.0 * (1.0 - %s.r) * (1.0 - %s.r); }",
		                                                   col1, res_r, col1, col2, res_r, col1, col2));
		parser_material_write(parser_material_kong, string_tmp("if (%s.g < 0.5) { %s = 2.0 * %s.g * %s.g; } else { %s = 1.0 - 2.0 * (1.0 - %s.g) * (1.0 - %s.g); }",
		                                                   col1, res_g, col1, col2, res_g, col1, col2));
		parser_material_write(parser_material_kong, string_tmp("if (%s.b < 0.5) { %s = 2.0 * %s.b * %s.b; } else { %s = 1.0 - 2.0 * (1.0 - %s.b) * (1.0 - %s.b); }",
		...
		char *v3 = string_tmp("(float3(1.0, 1.0, 1.0) - float3(%s, %s, %s)) * %s + float3(%s, %s, %s) * %s / %s", fac_var, fac_var, fac_var, col1, fac_var, fac_var,
		                  fac_var, col1, col2);
